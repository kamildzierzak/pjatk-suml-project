...
if BACKBONE == 'resnet':
    ...
    raise ValueError("Invalid BACKBONE. Choose from 'resnet', 'mobilenet', or 'efficientnet'.")
...
model = models.Sequential([
    ...
    layers.Dense(len(train_generator.class_indices), activation='softmax')  # Output layer (softmax for multi-class)
])
...
checkpoint = ModelCheckpoint(os.path.join(OUTPUT_DIR, f'best_{BACKBONE}_based_model.keras'), monitor='val_accuracy', save_best_only=True, mode='max')

## Reduce learning rate if the validation loss plateaus
lr_scheduler = ReduceLROnPlateau(monitor='val_loss', factor=0.75, patience=10, min_lr=1e-6)
...
checkpoint_fine_tuned_model = ModelCheckpoint(os.path.join(OUTPUT_DIR, f'best_fined_tuned_{BACKBONE}_based_model.keras'), monitor='val_accuracy', save_best_only=True, mode='max')
...
add_progress_visualization(history_fine_tune)

# Save the fine-tuned model
model.save(os.path.join(OUTPUT_DIR, f"constellation_recognition_fine_tuned_{BACKBONE}_model.keras"))
